...
/* ⚠ happy-dom se importa AQUÍ y no dentro de la prueba que lo usa («el collectAst REAL…»). Allí era un
   ...
   entera en paralelo la carga compite con los demás workers, y con Node 20 —el que usaba entonces la CI—
   expiró a los 5013 ms (medido el 2026-09-18). Una prueba que cae por el reloj tumba la CI y, con ella, el
   despliegue. En el nivel del módulo la carga es parte de la RECOLECCIÓN, que no tiene tope de tiempo. El archivo
   NO pasa al entorno happy-dom a propósito: con él, `URL` es la de happy-dom y `readFileSync` la rechaza («The URL
   must be of scheme file», medido) y el archivo entero cae al recolectarse. ⚠ Y no escribir aquí la directiva de
   entorno tal cual, ni siquiera citada: Vitest la busca en CUALQUIER comentario del archivo y la aplicaría. */
